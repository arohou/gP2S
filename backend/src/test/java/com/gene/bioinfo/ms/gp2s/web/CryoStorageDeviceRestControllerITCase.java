...
public class CryoStorageDeviceRestControllerITCase extends BaseRestITCase<CryoStorageDevice> {
    @Autowired
    private CryoStorageDeviceRepository repository;
    ...
    protected String createRequestBody(String label) throws IOException {
        return json(CryoStorageDevice.builder().label(label)
                 .manufacturer("XYZ").location("Village in the north").model("XXL")
                 .hasCylinders(true).hasTubes(true).hasBoxes(true).build());
    }
}
