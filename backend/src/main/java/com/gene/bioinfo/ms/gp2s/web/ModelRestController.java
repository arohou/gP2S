...
@RestController
@Api(description = "microscopy session management", value = "Model")
@RequestMapping(value = API_V1 + "/model", produces = APPLICATION_JSON_UTF8_VALUE)
public class ModelRestController extends BaseProjectRestController<Model, ModelService> implements
        ISingleAttachmentRestController {

    private final ISingleAttachmentRestController attachmentController;

    ModelRestController(
            @NonNull final ISingleAttachmentRestController modelAttachmentRestController,
            final ModelService service,
            final ModelValidator validator) {
        super(service, validator);
        this.attachmentController = modelAttachmentRestController;
    }
