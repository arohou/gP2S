...
@Service
public class MapService extends BaseProjectRestService<Map> {


    MapService(final MapRepository MapRepository,
               final ProjectRepository projectRepository) {
        super(MapRepository, projectRepository);
    }

    @Override
    public Collection<Project> getItemProjects(@NonNull final String slug) {
        return Optional.ofNullable(this.projectRepository.findByMap_Slug(slug))
                .orElse(Collections.emptySet());
    }

    @Override
    public Collection<Project> getItemProjects(@NonNull final Long id) {
        return Optional.ofNullable(this.projectRepository.findByMap_Id(id))
                .orElse(Collections.emptySet());
    }
}
