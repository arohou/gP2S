...
<template>
  <form-component>
    <actions-header slot="action-header" :title="'Editing Settings'"
...
<script>
  ...
  export default {
    ...
    methods: {
      saveForm () {
        return this._settingsService.save(this.entity)
          .then(result => {
            this.openViewForm('admin-admin-settings', result.data, true)
          })
