...
<template>
  <form-component>
    <actions-header slot="action-header" :title="'Creating Negative Stain Protocol'" @actionConfirm="submitBaseFormBy('mainForm')" @actionCancel="cancel('mainForm')"></actions-header>
...
<script>
  ...
  import BaseForm from '@/components/App/Admin/NegativeStainProtocol/BaseForm'
  ...
  export default {
    name: 'negative-stain-protocol-new',
    ...
    methods: {
      saveForm () {
        return service.createEntity(this.entity)
                      .then(result => {
                        this.openViewForm('admin-protocols-negative_stain', result.data, this.useSlug)
                      })
      },
      ...
      loadFormData () {
        this.entity = {
          id: '',
          label: this.defaultNewLabel('Negative stain protocol'),
          name: '',
          ph: '',
          concentration: '',
          temperature: '',
