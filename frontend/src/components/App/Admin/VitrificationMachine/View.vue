...
<template>
  <view-comment-base entityType="vitrification_machine" :entity="entity" :icon="icon">
    <el-row>
      <el-col :sm="8">
        <el-row>
...
<script>
  ...
  import Icon from '@/assets/images/vitrification-machine.svg'
  import ViewCommentBase from '@/components/App/ViewCommentBase'

  export default {
    name: 'vitrification-machine-view',
    ...
    mounted () {
      ...
      this.$events.on('actionEdit', data => this.actionEdit('admin-equipment-vitrification_machines', this.id))
      this.$events.on('actionCopy', data => this.actionCopy('admin-equipment-vitrification_machines', this.id))
    },
