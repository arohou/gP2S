...
<template>
  <form-component>
    <actions-header slot="action-header" :title="'Creating Blotting Paper'" @actionConfirm="submitBaseFormBy('mainForm')" @actionCancel="cancel('mainForm')"></actions-header>
...
<script>
  ...
  import BaseForm from '@/components/App/Admin/BlottingPaper/BaseForm'
    ...
    methods: {
      saveForm () {
        return service.createEntity(this.entity)
          .then(result => {
            this.openViewForm('admin-consumables-blot_paper_types', result.data, this.useSlug);
          })
      },
      ...
      loadFormData () {
        this.entity = {
          id: '',
          label: this.defaultNewLabel('Blotting paper'),
          manufacturer: '',
          model: ''
        }
      }
