...
<template>
  <div class="grid-type-list-view list-view scrollable-container">
    <div class="list-view__header">
      <el-button class="list-view__action-buttons__submit"
                 @click="$router.push({ name: 'admin-consumables-grid_type-new' })">
    ...
    <el-col class="list-view__components scrollable" :xs="15">
      <el-row v-for="(entity, index) in entities"
              :key="index">
        <dashboard-list-item :entity="entity" entityType="GRID_TYPE">
          <template slot="left-pane">
            <el-row class="list-view__link-label-wrapper">
              <router-link :to="{ name: 'admin-consumables-grid_type-view', params: {id: entity.slug}}"
                           class="list-view__link-label" :id="'grid_type-'  + index + '-label-link'">{{ entity.label }}
              </router-link>
            </el-row>
          </template>
