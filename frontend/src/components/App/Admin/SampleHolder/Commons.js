...
export default {
  ...
  filters: {
    sortMicrsocopeLabels (micrsocopes) {
      if (!micrsocopes) {
        return ''
      }
      let sortedLabels = micrsocopes.map(micrsocope => (micrsocope || {}).label)
      return sortedLabels.sort((a, b) => (a || '').toLowerCase() > (b || '').toLowerCase()).join(', ')
    }
  }
}
