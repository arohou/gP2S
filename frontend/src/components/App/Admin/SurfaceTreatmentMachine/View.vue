...
<template>
  <view-comment-base entityType="surface_treatment_machine" :entity="entity" :icon="icon">
    <el-row>
      <el-col :sm="8">
        <el-row>
...
<script>
  ...
  import Icon from '@/assets/images/surface-treatment-machine.svg'
  import ViewCommentBase from '@/components/App/ViewCommentBase'

  export default {
    name: 'surface-treatment-machine-view',
    ...
    mounted () {
      ...
      this.$events.on('actionEdit', data => this.actionEdit('admin-equipment-surface_treatment_machines', this.id))
      this.$events.on('actionCopy', data => this.actionCopy('admin-equipment-surface_treatment_machines', this.id))
    },
