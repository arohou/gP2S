...
<template>
  <div class="surface-treatment-machine-list-view list-view scrollable-container">
    <div class="list-view__header">
      <el-button class="list-view__action-buttons__submit"
                 @click="$router.push({ name: 'admin-equipment-surface_treatment_machines-new' })">
        Create new machine
      </el-button>
    </div>

    <el-col class="surface-treatment-machine-view__components scrollable" :xs="15">
      <el-row v-for="(entity, index) in entities"
              :key="index">
        <dashboard-list-item :entity="entity" entityType="SURFACE_TREATMENT_MACHINE">
          <template slot="left-pane">
            <el-row class="list-view__link-label-wrapper">
              <router-link :to="{ name: 'admin-equipment-surface_treatment_machines-view', params: {id: entity.slug}}"
