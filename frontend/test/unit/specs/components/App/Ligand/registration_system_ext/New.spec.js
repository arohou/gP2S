...
import New from '@/components/App/Ligand/registration_system_ext/New'
...
describe('Ligand/New.vue', () => {
  ...
  it('should create new ligand form have save and cancel buttons enabled', done => {
    Vue.nextTick().then(() => {
  ...
  it('should create new ligand form be default', done => {
    Vue.nextTick().then(() => {
      const expectedLabel = 'Ligand 17-08-01 08:13'
      expect([...vm.$el.querySelectorAll('input')].map(x => x.value)).to.be.eql([expectedLabel, '', '', '', '',
        'DMSO'])
    }).then(done, done)
  })
